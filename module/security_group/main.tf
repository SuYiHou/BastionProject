// -----------------------------------------------------------------------------
// Security Group 模块：根据 map 输入统一创建 N 组安全组以及对应的 ingress/egress 规则。
//  - 优势：所有规则集中管理，可引用 SG 名称，自动展开为 terraform 资源。
//  - bastion/eks/private_compute 等都可以在这里定义，后续模块只需按 key 取值。
// -----------------------------------------------------------------------------
locals {
  // 统一标签，便于计费/排错
  base_tags = merge({
